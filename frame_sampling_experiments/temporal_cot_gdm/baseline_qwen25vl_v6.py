"""
baseline_qwen25vl_v6.py — Deterministic Vanilla Qwen2.5-VL-7B Baseline (v6).

Raw video file → Qwen2.5-VL native video input → answer.
No frame extraction, no TCoT, no context curation.
Fully deterministic: fixed seeds, do_sample=False.

Results:  results/<dataset>/Qwen2.5-VL-7B_baseline_native_v6_k256_results_v6.jsonl
"""
...
logger = logging.getLogger("baseline.qwen25vl_v6")
...
class NativeQwen25VLInference:
    ...
    def load(self):
        logger.info("[Qwen2.5VL] Loading %s …", self._MODEL_ID)
        ...
        logger.info("[Qwen2.5VL] Model loaded.")
...
def main():
    logger.info("=" * 70)
    logger.info("Deterministic Baseline v6 — Qwen2.5-VL-7B (native video)")
    logger.info("  Model       : %s", NativeQwen25VLInference._MODEL_ID)
