...
try:
    from trl.trainer.utils import pad_to_length  # TRL <= 0.x
except ImportError:
    # TRL >= 1.x removed pad_to_length; provide a drop-in replacement.
...
class QwenDPOTrainer(DPOTrainer):
    ...
    def _get_train_sampler(self, *args, **kwargs):
        """
        Curriculum-friendly sampler override.
        When NO_SHUFFLE_TRAIN=1 is set in the environment, return a
        SequentialSampler so a pre-sorted (curriculum-ordered) train set
        is consumed in its on-disk order.  HF Accelerate's DataLoader
        wrapper handles distributed sharding around this sampler, so
        the *global* iteration order remains roughly easy->hard across
        all DP ranks.
        Otherwise, fall back to the parent (default = RandomSampler).
        """
        if os.environ.get("NO_SHUFFLE_TRAIN", "") in ("1", "true", "True"):
            from torch.utils.data import SequentialSampler
            print("[QwenDPOTrainer] NO_SHUFFLE_TRAIN=1  ->  SequentialSampler "
                  f"(n={len(self.train_dataset)})", flush=True)
