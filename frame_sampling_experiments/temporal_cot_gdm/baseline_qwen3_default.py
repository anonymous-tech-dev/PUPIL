...
══════════════════════════════════════════════════════════════════════════════
  ...
  NUM_SAMPLES    — -1 = all
  NATIVE_FPS     — fps passed to Qwen's video pratyat
  tokeniser (default: 1.0,
...
# FPS passed to Qwen's video tokeniser (2.0 = qwen_vl_utils default).
NATIVE_FPS = 2.0

# Max pixels per frame. None = let Qwen auto-scale (stock settings).
NATIVE_MAX_PIXELS = None

# Variant tag for the results file (separate from capped run)
NATIVE_VARIANT = "baseline_native_default"
...
class NativeQwenInference:
    ...
    def load(self):
        logger.info("[NativeQwen] Loading %s …", self._MODEL_ID)
    ...
    def infer(self, video_path: str, prompt: str) -> str:
        ...
        if NATIVE_MAX_PIXELS is not None:
            video_content["max_pixels"] = NATIVE_MAX_PIXELS
        ...
        # 1. Ask for BOTH kwargs and metadata
        image_inputs, video_inputs, video_kwargs = process_vision_info(
            messages, image_patch_size=16, return_video_kwargs=True, return_video_metadata=True
        )
        
        # 2. Unzip the tuples inside video_inputs and inject them into video_kwargs
        ...
        # 3. Pass to processor (which will now correctly see video_metadata inside **video_kwargs)
        inputs = self.processor(
            text=[text],
            images=image_inputs,
            videos=video_inputs,
