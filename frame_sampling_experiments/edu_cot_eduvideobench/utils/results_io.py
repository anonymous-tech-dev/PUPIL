...
def _shard_suffix(cfg: DictConfig) -> str:
    n = int(getattr(cfg, "num_shards", 1) or 1)
    if n <= 1:
        return ""
    sid = int(getattr(cfg, "shard_id", 0) or 0)
    return f"_shard{sid}of{n}"


def results_filepath(cfg: DictConfig) -> str:
    """Return the full path to the results JSONL file."""
    results_dir = os.path.join(
        os.path.dirname(os.path.dirname(__file__)),  # edu_cot_Pupil/
        cfg.results_dir,
        cfg.dataset.name,
    )
    ...
    if not adapter_tag:
        # Fall back to env var, then basename of adapter_dir
        import os as _os
        adapter_tag = _os.environ.get("ADAPTER_TAG", "")
        if not adapter_tag:
            ad = (cfg.model.get("adapter_dir") or "") if hasattr(cfg.model, "get") else ""
            if ad:
                adapter_tag = _os.path.basename(ad.rstrip("/"))
    if adapter_tag:
        model_tag = f"{model_tag}_ft_{adapter_tag}"

    variant = build_variant_tag(cfg)
    suffix = _shard_suffix(cfg)
    return os.path.join(results_dir, f"{model_tag}_{variant}{suffix}_results.jsonl")
