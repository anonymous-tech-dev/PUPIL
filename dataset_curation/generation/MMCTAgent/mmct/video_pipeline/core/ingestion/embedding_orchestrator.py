...
class EmbeddingOrchestrator:
    ...
    async def _retry_with_backoff(self, func, *args, **kwargs):
        ...
        Retries on Exceptions OR if the returned data is None/Empty.
        """
        max_retries = 5
        base_delay = 2
        ...
        for attempt in range(max_retries):
            try:
                ...
                # CRITICAL: If result is None, treat it as a failure!
                if result is None:
                    raise ProviderException("Provider returned None")
        ...
        try:
            ...
            try:
                ...
                for i in range(0, len(valid_file_paths), batch_size):
                    ...
                    # 1. Define the specific call
                    async def call_provider():
                        # Call the provider
                        res = await embeddings_generator.provider.batch_image_embedding(batch_paths)
                        
                        # 2. VALIDATION: Check for the "Silent Failure"
                        if not res:
                            raise ProviderException("Provider returned empty list")
                        if len(res) != len(batch_paths):
                            raise ProviderException(f"Mismatch! Sent {len(batch_paths)} images, got {len(res)} embeddings.")
                        return res

                    # Generate embeddings for this batch
                    # batch_embeddings = await embeddings_generator.provider.batch_image_embedding(batch_paths)
                    batch_embeddings = await self._retry_with_backoff(call_provider)
        ...
        try:
            ...
            for chapter in chapter_collection.chapters:
                ...
                embedding_tasks.append(
                    self._retry_with_backoff(create_embedding, chapter_content)
                )
    ...
    async def _generate_object_collection_embeddings(self, video_id: str) -> None:
        ...
        try:
            ...
            embedding = await self._retry_with_backoff(
                create_embedding, 
                object_collection_metadata.video_summary
            )
