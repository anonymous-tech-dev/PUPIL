...
class DPODataset(Dataset):
    ...
    def __init__(
        ...
        # CRITICAL: forward per-clip pixel budget and frame cap to get_video_info
        # so qwen-vl-utils does not default to its full-context budget (which
        # would otherwise produce ~50k vision patches per frame and stall the
        # entire dataloader). The SFT dataset already passes these; the DPO
        # dataset previously omitted them.
        ...
        # Resolve model family from the actual config (see sft_dataset.py for the
        # full rationale: substring matching on model_id silently misclassifies
        # merged-adapter checkpoints whose path doesn't contain 'Qwen3', producing
        # patch-shape mismatches inside the Qwen3-VL video processor).
        try:
        ...
        if (self._model_family == "qwen3") != _path_says_q3:
            print(
                f"[SoFDPODataset] model_id path={self.model_id!r} substring-says-Qwen3={_path_says_q3} "
...
def make_dpo_data_module(model_id, processor, data_args):
    ...
    eval_dataset = None
    eval_path = getattr(data_args, "eval_path", None)
    if eval_path:
        eval_dataset = DPODataset(
            data_path=eval_path, processor=processor, data_args=data_args, model_id=model_id
        )
