...
def run_pipeline(model, item: Dict[str, Any], cfg: DictConfig) -> Dict[str, Any]:
    ...
    """
    t_start = time.time()
    # Per-stage wall-clock accumulators (seconds) — persisted for paper tables.
    timings: Dict[str, float] = {
        "prep_secs":        0.0,   # video meta + segmentation + candidate pool
        "selection_secs":   0.0,
    ...
    # Wall-clock breakpoint: everything before this is video prep + segmentation.
    timings["prep_secs"] = round(time.time() - t_start, 2)
    _t_select = time.time()
    ...
    timings["selection_secs"] = round(time.time() - _t_select, 2)
    ...
    timings["aggregation_secs"] = round(time.time() - _t_agg, 2)
    ...
    timings["answering_secs"] = round(time.time() - t_ans, 2)
    timings["total_secs"]     = round(time.time() - t_start, 2)
    ...
    return {
        ...
        # ── Provenance (so concatenated SFT-run JSONLs stay disambiguable) ──
        "model_name":        cfg.model.name,
        "adapter_dir":       (cfg.model.get("adapter_dir") or "") if hasattr(cfg.model, "get") else "",
        "adapter_tag":       (cfg.model.get("adapter_tag") or "") if hasattr(cfg.model, "get") else "",
        # ── Per-stage timings (seconds) for paper tables ──
        "timings":           timings,
        # selection-stage VLM rounds + 1 final answer call
        "num_vlm_calls":     len(all_raw_responses) + 1,
        # Fraction of all video frames that survived selection (TCoT parity).
        "pct_selected":      (
            100.0 * len(all_selected_ids) / meta.total_frames
            if meta.total_frames and meta.total_frames > 0 else -1.0
        ),
    }
